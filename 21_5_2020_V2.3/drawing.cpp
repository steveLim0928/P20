...
void drawing::mousePressEvent(QMouseEvent *event) // start drawing
{
    if (!texting) {
        ...
        if (event->button() == Qt::LeftButton) {
            text(event->pos());
            emit(sendData(pointSerial.serial(event->pos()), charSerial.serial('E')));
        }
    }
}
...
void drawing::createMenu() { // Top menu bar
    ...
    brushMenu->addAction(textAction);
    brushMenu->addAction(textSizeAction);
...
void drawing::createAction() { // smaller menu bar child option command
    ...
    textAction = new QAction(tr("&Add Text"), this);
    connect(textAction, &QAction::triggered, this, &drawing::getText);

    textSizeAction = new QAction(tr("&Change Text Size"), this);
    connect(textSizeAction, &QAction::triggered, this, &drawing::changeTextSize);
...
void drawing::getText() {

    QString text = QInputDialog::getText(this, tr("Text"), tr("Enter Your Text:"));
    // within range
    ...
    //serialise
    emit(sendData(stringSerial.serial(displayText), charSerial.serial('T')));

}
void drawing::changeTextSize() {
    bool check;
    int newSize = QInputDialog::getInt(this, tr("Size"),
                                       tr("Enter Text size:"),
                                       fontSize,
    ...
    if (check) {
        ...
        //serialise
        emit(sendData(intSerial.serial(fontSize), charSerial.serial('s')));
    }
}
...
void drawing::onTextPressed(QByteArray data) {
    text(pointSerial.deserial(data));
}
void drawing::onSetText(QByteArray data) {
    displayText = stringSerial.deserial(data);
}
void drawing::onSetTextSize(QByteArray data) {
    fontSize = intSerial.deserial(data);
}
...
void drawing::clear() // clear the canvas
{
    ...
    emit(sendData(0,charSerial.serial('C')));
}
...
void drawing::text(const QPoint &pos) {
    QPainter painter(&canvasTemplate);
    ...
    painter.drawText(pos, displayText);
    texting = 0;
    update();
}
